...
export default (isNextMove, checkersPosition, from, to) => {
  ...
  const indexOfChecker = checkersPosition.indexOf(
    checkersPosition.filter(
      item => item.col === from.col && item.row === from.row
    )[0]
  );

  if (
    (to.row === 7 && from.color === "white") ||
    (to.row === 0 && from.color === "black")
  ) {
  ...
  if (hasNextMove) {
    const selected = {
      ...
      row: to.row,
      color: from.color,
      isKing: checkersPosition[indexOfChecker].isKing
    };
    let { availablePoles, containsDoubleMove } = getActivePoles(
      selected,
      checkersPosition,
      isNextMove,
      from
