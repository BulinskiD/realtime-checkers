export default (
  ...
  const arrays = getPolesAround(
    checkersPosition,
    col,
    availableRow,
    nextAvailableRow
  );
  let [containsDoubleMove, singleMove, doubleMove] = checkLeftAndRight(
    arrays,
    availableRow,
    nextAvailableRow,
    col
  ...
  if (isKing) {
    ...
    const kingsArrays = getPolesAround(
      checkersPosition,
      col,
      kingRow,
      nextKingRow
    ...
    const [
      containsKingsDoubleMove,
      kingSingleMove,
      kingDoubleMove
    ] = checkLeftAndRight(kingsArrays, kingRow, nextKingRow, col);
  ...
  availablePoles = availablePoles.filter(
  ...
  if (from)
    availablePoles = availablePoles.filter(
      item => !(item.row === from.row && item.col === from.col)
    );
...
const filterArrayToGetPole = (checkersPosition, col, availableRow, side) => {
  return checkersPosition.filter(
    item => item.col === col + side && item.row === availableRow
  );
};
...
const getPolesAround = (
  checkersPosition,
  col,
  availableRow,
  nextAvailableRow
) => {
  const oneToRight = filterArrayToGetPole(
    checkersPosition,
    col,
    availableRow,
    1
  );
  const twoToRight = filterArrayToGetPole(
    checkersPosition,
    col,
    nextAvailableRow,
    2
  ...
  const oneToLeft = filterArrayToGetPole(
    checkersPosition,
    col,
    availableRow,
    -1
  );
  const twoToLeft = filterArrayToGetPole(
    checkersPosition,
    col,
    nextAvailableRow,
    -2
