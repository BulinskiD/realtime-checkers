...
describe("GamesReducer", () => {
  it("should set id from action payload on SELECT_GAME action", () => {
    expect(
      gamesReducer(initialState, { type: SELECT_GAME, payload: "2" })
    ).toStrictEqual({ ...initialState, id: "2" });
  });

  it("should return new state with selectedChecker for SELECT_CHECKER action", () => {
    const checkersPosition = [
      { col: 2, row: 2, selected: false },
      { col: 3, row: 3, selected: false }
    ];
    initialState.checkersPosition = checkersPosition;

    expect(
      gamesReducer(initialState, {
        type: SELECT_CHECKER,
        payload: { col: 2, row: 2 }
      })
    ).toStrictEqual({
      ...initialState,
      checkersPosition,
      selectedChecker: { col: 2, row: 2, selected: true }
    });
  });
  ...
  it("should set new state for SET_NEW_STATE action", () => {
    ...
    action.payload.gameState.selectedChecker = {
      col: 2,
      row: 2,
      selected: true
    };

    expect(gamesReducer(initialState, action)).toStrictEqual({
  ...
  it("should return initial state for other action", () => {
    expect(gamesReducer(initialState, { type: "" })).toStrictEqual({
      ...initialState
    });
  });
});
